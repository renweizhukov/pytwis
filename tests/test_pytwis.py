...
class PytwisTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        try:
            ...
            self.fail('Failed to connect to the Redis server: {}'.format(str(excep)))

        self._pytwis._rc.flushdb()  # pylint: disable=protected-access
...
class PytwisTestsWithRegisteredUsers(PytwisTests):
    ...
    def setUp(self):
        """After PytwisTests.setUp(), we register some new users to prepare
...
class PytwisLogTests(PytwisTestsWithRegisteredUsers):
    ...
    def _login_with_empty_username(self):
        ...
        self.assertEqual(pytwis.pytwis_constants.ERROR_USERNAME_NOT_EXIST_FORMAT.\
                            format(''),
                         result[pytwis.pytwis_constants.ERROR_KEY],
                         INCORRECT_ERROR_MSG)
    ...
    def _login_with_wrong_username(self):
        ...
        self.assertFalse(succeeded,
                         'Succeeded in logging in with a wrong username {}'.\
    ...
    def _login_with_correct_password(self):
        ...
        self.assertGreater(len(result[pytwis.pytwis_constants.AUTH_KEY]), 0,
                           'login returns an empty authentication secret')
    ...
    def _login_with_empty_password(self):
        ...
        self.assertFalse(succeeded,
                         'Succeeded in logging into {} with an empty password'.\
                            format(self._usernames[0]))
        self.assertEqual(pytwis.pytwis_constants.ERROR_INCORRECT_PASSWORD,
                         result[pytwis.pytwis_constants.ERROR_KEY],
                         INCORRECT_ERROR_MSG)
    ...
    def _login_with_wrong_password(self):
        ...
        succeeded, result = self._pytwis.login(self._usernames[0],
                                               self._passwords[0] + '_wrong')
        self.assertFalse(succeeded,
                         'Succeeded in logging into {} with wrong password'.\
                            format(self._usernames[0]))
        self.assertEqual(pytwis.pytwis_constants.ERROR_INCORRECT_PASSWORD,
                         result[pytwis.pytwis_constants.ERROR_KEY],
                         INCORRECT_ERROR_MSG)
    ...
    def _logout_before_login(self):
        ...
        self.assertEqual(pytwis.pytwis_constants.ERROR_NOT_LOGGED_IN,
                         result[pytwis.pytwis_constants.ERROR_KEY],
                         INCORRECT_ERROR_MSG)
...
class PytwisChangePasswordTests(PytwisTestsWithRegisteredUsers):
    ...
    def _change_password_with_wrong_old_password(self):
        ...
        succeeded, result = self._pytwis.change_password(auth_secret,
                                                         self._passwords[0] + '_wrong',
                                                         '')
        self.assertFalse(succeeded,
                         'Succeeded in changing the password with incorrect old password')
        self.assertEqual(pytwis.pytwis_constants.ERROR_INCORRECT_OLD_PASSWORD,
                         result[pytwis.pytwis_constants.ERROR_KEY],
                         INCORRECT_ERROR_MSG)
    ...
    def _change_password_with_same_new_password(self):
        ...
        succeeded, result = self._pytwis.change_password(auth_secret,
                                                         ...
                                                         self._passwords[0])
        self.assertFalse(succeeded,
                         'Succeeded in changing the password with same new password')
        self.assertEqual(pytwis.pytwis_constants.ERROR_NEW_PASSWORD_NO_CHANGE,
                         result[pytwis.pytwis_constants.ERROR_KEY],
                         INCORRECT_ERROR_MSG)
    ...
    def _change_password_with_weak_new_password(self):
        ...
        succeeded, result = self._pytwis.change_password(auth_secret,
                                                         ...
                                                         '12345678')
        self.assertFalse(succeeded,
                         'Succeeded in changing the password with a weak new password')
        self.assertEqual(pytwis.pytwis_constants.ERROR_WEAK_PASSWORD,
                         result[pytwis.pytwis_constants.ERROR_KEY],
                         INCORRECT_ERROR_MSG)
    ...
    def _change_password_after_login_then_logout_login(self):
        ...
        succeeded, result = self._pytwis.change_password(old_auth_secret,
                                                         old_password,
                                                         new_password)
...
class PytwisUserProfileTests(PytwisTestsWithRegisteredUsers):
    ...
    def _get_user_profile_with_empty_auth_secret(self):
        ...
        self.assertFalse(succeeded,
                         '''Succeeded in getting the user profile with
                         an empty authentication secret''')
        self.assertEqual(pytwis.pytwis_constants.ERROR_NOT_LOGGED_IN,
                         result[pytwis.pytwis_constants.ERROR_KEY],
                         INCORRECT_ERROR_MSG)
    ...
    def _get_user_profile_with_incorrect_auth_secret(self):
        ...
        self.assertFalse(succeeded,
                         ''''Succeeded in getting the user profile with
                         an incorrect authentication secret''')
        self.assertEqual(pytwis.pytwis_constants.ERROR_NOT_LOGGED_IN,
                         result[pytwis.pytwis_constants.ERROR_KEY],
                         INCORRECT_ERROR_MSG)
    ...
    def _get_user_profile_with_correct_auth_secret(self):
        ...
        self.assertTrue(succeeded,
                        'Failed to get the user profile with the correct authentication secret')
        self.assertEqual(self._usernames[0],
                         result[pytwis.pytwis_constants.USERNAME_KEY],
                         'Mismatched username')
        self.assertTrue(check_password_hash(result[pytwis.pytwis_constants.PASSWORD_HASH_KEY],
                                            self._passwords[0]), 'Mismatched password')
        self.assertEqual(self._auth_secret,
                         result[pytwis.pytwis_constants.AUTH_KEY],
                         'Mismatched authentication secret')
...
class PytwisTimelineTests(PytwisTestsWithRegisteredUsers):
    ...
    def _get_empty_timeline(self):
        ...
        self.assertEqual(0, len(result[pytwis.pytwis_constants.TWEETS_KEY]),
                         'Get a non-empty timeline')

    def _get_nonempty_timeline_all(self):
        """Get all the tweets of a nonempty timeline."""
        for tweet_index in range(0, self._cnt_tweets):
            succeeded, _ = self._pytwis.post_tweet(self._post_auth_secret,
                                                   'Test tweet {}'.format(tweet_index))
        ...
        self.assertEqual(self._cnt_tweets,
                         len(result[pytwis.pytwis_constants.TWEETS_KEY]),
                         'Get an incorrect number of tweets')
    ...
    def _get_nonempty_timeline_fewer(self):
        ...
        self.assertEqual(self._cnt_tweets // 2,
                         len(result[pytwis.pytwis_constants.TWEETS_KEY]),
                         'Get an incorrect number of tweets')
...
class PytwisFollowTests(PytwisTestsWithRegisteredUsers):
    ...
    def setUp(self):
        ...
        for user_index in range(0, self.CNT_REGISTERED_USERS):
            succeeded, result = self._pytwis.login(self._usernames[user_index],
    ...
    def _follow_unfollow_wrong_username_after_login(self):
        ...
        self.assertFalse(succeeded,
                         'Succeeded in following a wrong username {}'.format(wrong_followee))
        self.assertEqual(pytwis.pytwis_constants.ERROR_FOLLOWEE_NOT_EXIST_FORMAT.\
                            format(wrong_followee),
                         result[pytwis.pytwis_constants.ERROR_KEY],
                         INCORRECT_ERROR_MSG)

        succeeded, result = self._pytwis.unfollow(self._auth_secrets[0], wrong_followee)
        self.assertFalse(succeeded,
                         'Succeeded in unfollowing a wrong username {}'.format(wrong_followee))
        self.assertEqual(pytwis.pytwis_constants.ERROR_FOLLOWEE_NOT_EXIST_FORMAT.\
                            format(wrong_followee),
                         result[pytwis.pytwis_constants.ERROR_KEY],
                         INCORRECT_ERROR_MSG)
    ...
    def _follow_yourself_after_login(self):
        ...
        self.assertFalse(succeeded,
                         'Succeeded in following yourself {}'.format(self._usernames[0]))
        self.assertEqual(pytwis.pytwis_constants.ERROR_FOLLOW_YOURSELF_FORMAT.\
                            format(self._usernames[0]),
                         result[pytwis.pytwis_constants.ERROR_KEY],
                         INCORRECT_ERROR_MSG)
    ...
    def _one_follow_multiple(self):
        ...
        for followee_index in range(1, self.CNT_REGISTERED_USERS):
            succeeded, _ = self._pytwis.follow(self._auth_secrets[0],
                                               self._usernames[followee_index])
            ...
            self.assertTrue(succeeded,
                            'Failed to get the follower list for {}'.\
                                format(self._usernames[followee_index]))
            self.assertEqual(1, len(result[pytwis.pytwis_constants.FOLLOWER_LIST_KEY]),
                             'Incorrect number of followers for {}'.\
                                format(self._usernames[followee_index]))

        succeeded, result = self._pytwis.get_following(self._auth_secrets[0])
        self.assertTrue(succeeded,
                        'Failed to get the following list for {}'.format(self._usernames[0]))
        self.assertEqual(self.CNT_REGISTERED_USERS - 1,
                         len(result[pytwis.pytwis_constants.FOLLOWING_LIST_KEY]),
                         'Incorrect number of followings for {}'.format(self._usernames[0]))

        for followee_index in range(1, self.CNT_REGISTERED_USERS):
            succeeded, _ = self._pytwis.unfollow(self._auth_secrets[0],
                                                 self._usernames[followee_index])
    ...
    def _multiple_follow_one(self):
        ...
        for follower_index in range(1, self.CNT_REGISTERED_USERS):
            succeeded, _ = self._pytwis.follow(self._auth_secrets[follower_index],
                                               self._usernames[0])
            self.assertTrue(succeeded, '{} failed to follow {}'.\
            ...
            self.assertTrue(succeeded,
                            'Failed to get the following list for {}'.\
                                format(self._usernames[follower_index]))
            self.assertEqual(1,
                             len(result[pytwis.pytwis_constants.FOLLOWING_LIST_KEY]),
                             'Incorrect number of followings for {}'.\
                                format(self._usernames[follower_index]))

        succeeded, result = self._pytwis.get_followers(self._auth_secrets[0])
        self.assertTrue(succeeded,
                        'Failed to get the follower list for {}'.format(self._usernames[0]))
        self.assertEqual(self.CNT_REGISTERED_USERS - 1,
                         len(result[pytwis.pytwis_constants.FOLLOWER_LIST_KEY]),
                         'Incorrect number of followers for {}'.format(self._usernames[0]))

        for follower_index in range(1, self.CNT_REGISTERED_USERS):
            succeeded, _ = self._pytwis.unfollow(self._auth_secrets[follower_index],
                                                 self._usernames[0])
            self.assertTrue(succeeded, '{} failed to unfollow {}'.\
                            format(self._usernames[follower_index], self._usernames[0]))
...
class PytwisPostFollowTests(PytwisTestsWithRegisteredUsers):
    ...
    def _post_after_two_follows(self):
        ...
        for user_index in range(self.CNT_REGISTERED_USERS):
            ...
            self.assertTrue(succeeded,
                            'Failed to get the user timeline of {}'.\
                                format(self._usernames[user_index]))
            self.assertEqual(self._expected_cnt_tweets[user_index],
                             len(result[pytwis.pytwis_constants.TWEETS_KEY]),
                             'Incorrect number of tweets for {}'.\
                                format(self._usernames[user_index]))
    ...
    def _post_after_one_follow_and_one_unfollow(self):
        ...
        succeeded, _ = self._pytwis.post_tweet(self._auth_secrets[0],
                                               'Tweet after one follow and one unfollow.')
        self.assertTrue(succeeded, '{} failed to post a tweet'.format(self._usernames[0]))
        for user_index in range(self.CNT_REGISTERED_USERS):
...
class PytwisUserTweetsTests(PytwisTestsWithRegisteredUsers):
    ...
    def _get_user_tweets_without_login(self):
        ...
        self.assertFalse(succeeded,
                         '''Succeeded in getting the user tweets
                         with an empty authentication secret.''')
        self.assertEqual(pytwis.pytwis_constants.ERROR_NOT_LOGGED_IN,
                         result[pytwis.pytwis_constants.ERROR_KEY],
                         INCORRECT_ERROR_MSG)
    ...
    def _verify_user_tweets(self, cnt_tweets, username, tweets):
        ...
        """
        self.assertEqual(cnt_tweets, len(tweets),
                         'Incorrect number of tweets posted by {}'.format(username))
        for tweet in tweets:
            self.assertEqual(username,
                             tweet[pytwis.pytwis_constants.USERNAME_KEY],
                             'Mismatched username')
    ...
    def _get_user_tweets_without_follow(self):
        ...
        for user_index in range(self.CNT_REGISTERED_USERS):
            ...
            for tweet_index in range(0, self._expected_cnt_tweets[user_index]):
                succeeded, _ = self._pytwis.post_tweet(
                    self._auth_secrets[user_index],
                    'Tweet {} without follow'.format(tweet_index))
                self.assertTrue(succeeded, '{} failed to post a tweet'.\
                                format(self._usernames[user_index]))
    ...
    def _get_user_tweets_with_follow(self):
        ...
        succeeded, _ = self._pytwis.follow(self._auth_secrets[0], self._usernames[1])
        self.assertTrue(succeeded, '{} failed to follow {}'.format(self._usernames[0],
                                                                   self._usernames[1]))
    ...
    def _get_fewer_user_tweets(self):
        ...
        for user_index in range(self.CNT_REGISTERED_USERS):
            succeeded, result = self._pytwis.get_user_tweets(
                self._auth_secrets[0],
                self._usernames[user_index],
...
class PytwisSocketTests(unittest.TestCase):
    ...
    def test_connect_socket(self):
        ...
        try:
            ...
            self.fail('Failed to connect to the Redis server via socket: {}'.format(str(excep)))
